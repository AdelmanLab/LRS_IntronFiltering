...
while ( nrowremoved !=0 ) {
  ...
  # For the first time going through the loop, make two new columns filled with 0 or NA. If not, then remove the rows that were flagged. 
  if ( n == 0 ){
  ...
  for (i in 1:dim(loop_temp)[1]) {
    ...
    #Condition: If intron[i] and intron[i+1] have the same 5SS and 3SS, Add a +1 Flag to the one with with a larger intron number
    ...
    #Condition: If intron[i] and intron[i+1] have the same 5SS or 3SS, Add a +1 Flag to the annotation with fewer spliced LRS read counts 
